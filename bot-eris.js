...
bot.on('messageReactionAdd', async (event, emoji, userId) => {
    try {
...
node_schedule_1.scheduleJob("* * 19 * * 3", () => {
    postEventMsg();
});
node_schedule_1.scheduleJob("* * 19 * * 2", async () => {
    postOverview(0);
});
node_schedule_1.scheduleJob("* * 19 * * 6", async () => {
    postOverview(1);
});
...
//# sourceMappingURL=bot-eris.js.map